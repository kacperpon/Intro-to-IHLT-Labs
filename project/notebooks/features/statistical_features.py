...
def add_POS_statistics(df):
    ...
    for i in range(len(pos)):
        ...
        # Number of verbs
        df.loc[i, 's1_n_verbs_tot'] = len([word for word in pos[i][0] if word[1] in VERBS])
        df.loc[i, 's2_n_verbs_tot'] = len([word for word in pos[i][1] if word[1] in VERBS])
        # Verbs in present
        df.loc[i, 's1_n_verbs_pres'] = len([word for word in pos[i][0] if word[1] in VERBS_PRESENT])
        df.loc[i, 's2_n_verbs_pres'] = len([word for word in pos[i][1] if word[1] in VERBS_PRESENT])
        # Verbs in present
        df.loc[i, 's1_n_verbs_past'] = len([word for word in pos[i][0] if word[1] in VERBS_PAST])
        df.loc[i, 's2_n_verbs_past'] = len([word for word in pos[i][1] if word[1] in VERBS_PAST])
        # Number of nouns
